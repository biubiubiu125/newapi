...
func TestRegistrySetDisabledFactoryKeysHidesFactoryPlugin(t *testing.T) {
	...
	require.NoError(t, registry.SetDisabledFactoryKeys([]string{"factory-off"}))
	_, ok = registry.Get("factory-off")
	assert.False(t, ok)
	generation = registry.Generation()
	...
	require.NoError(t, registry.SetDisabledFactoryKeys(nil))
...
func TestRegistrySetDisabledFactoryKeysRetainsStateWhenRebuildFails(t *testing.T) {
	registry := NewRegistry()
	require.NoError(t, registerTestPlugin(registry, "1.0.0", true))
	require.NoError(t, registry.SetDisabledFactoryKeys(nil))

	var fail atomic.Bool
	require.NoError(t, registry.SetGenerationPreparer(func(candidate, current *RoutingGeneration) (PreparedRoutingGeneration, error) {
		if fail.Load() {
			return PreparedRoutingGeneration{}, fmt.Errorf("forced rebuild failure")
		}
	...
	err := registry.SetDisabledFactoryKeys([]string{"test"})
...
func TestRegistrySetDisabledFactoryKeysLeavesEnabledOverrideServing(t *testing.T) {
	...
	require.NoError(t, registry.SetDisabledFactoryKeys([]string{"test"}))
...
func TestRegistrySetDisabledFactoryKeysEmptySetDoesNotBumpGeneration(t *testing.T) {
	...
	require.NoError(t, registry.SetDisabledFactoryKeys(nil))
	assert.Equal(t, before, registry.Generation().Number)
	require.NoError(t, registry.SetDisabledFactoryKeys([]string{}))
	assert.Equal(t, before, registry.Generation().Number)
	require.NoError(t, registry.SetDisabledFactoryKeys([]string{"", "  "}))
	assert.Equal(t, before, registry.Generation().Number)
}
...
func TestRegistrySnapshotDisabledFactoryIsSorted(t *testing.T) {
	...
	require.NoError(t, registry.SetDisabledFactoryKeys([]string{"other", "test"}))
