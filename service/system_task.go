...
type SystemTaskLockTypeProvider interface {
	LockType() string
}
...
func systemTaskLockType(handler SystemTaskHandler) string {
	if provider, ok := handler.(SystemTaskLockTypeProvider); ok {
		return provider.LockType()
	}
	return handler.Type()
}
...
func runSystemTaskClaimPass(runnerID string) {
	...
	for _, handler := range handlers {
		...
		claimedTask, claimed, err := model.ClaimSystemTask(task.ID, handler.Type(), systemTaskLockType(handler), runnerID, systemTaskLockUntil())
		if err != nil {
