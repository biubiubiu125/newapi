...
const OAUTH_LOGIN_REDIRECT_KEY_PREFIX = 'oauth_login_redirect:'
...
function oauthLoginRedirectKey(provider: string, state: string): string {
  return `${OAUTH_LOGIN_REDIRECT_KEY_PREFIX}${provider}:${state}`
}
...
export function rememberOAuthLoginRedirect(
  ...
  const redirect = sanitizeAuthRedirect(redirectTo, origin)
  if (!storage || !provider || !state || !redirect) return false

  try {
    const key = oauthLoginRedirectKey(provider, state)
    storage.setItem(key, redirect)
    return storage.getItem(key) === redirect
  } catch {
    return false
  }
}

export function resolveOAuthLoginRedirectTarget(
  storage: OAuthModeStorage | null | undefined,
  provider: string,
  state: string,
  preferredRedirect: unknown,
  origin: string
): string | null {
  const preferred = sanitizeAuthRedirect(preferredRedirect, origin)
  if (preferred) return preferred
  if (!storage || !provider || !state) return null

  const key = oauthLoginRedirectKey(provider, state)
  try {
    const stored = storage.getItem(key)
    storage.removeItem?.(key)
    return sanitizeAuthRedirect(stored, origin)
  } catch {
    return null
  }
}
