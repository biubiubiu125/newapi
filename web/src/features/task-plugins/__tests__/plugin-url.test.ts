...
describe('browser plugin source fetch', () => {
  ...
  test('stops reading a chunked body as soon as it exceeds the limit', async () => {
    let reads = 0
    const stream = new ReadableStream<Uint8Array>({
      pull(controller) {
        ...
        if (reads === 1) {
          controller.enqueue(
            new TextEncoder().encode('x'.repeat(MAX_PLUGIN_SOURCE_BYTES))
          )
          return
        }
    ...
    await assert.rejects(
      fetchPluginSourceText('https://example.com/chunked.js', async () => response),
      (error: unknown) => {
        assert.ok(error instanceof PluginSourceFetchError)
        assert.equal(error.reason, 'too_large')
        return true
