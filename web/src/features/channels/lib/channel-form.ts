...
function formDataSupportsUpstreamModelUpdate(
  formData: Pick<ChannelFormValues, 'type' | 'multi_key_mode'>
): boolean {
  return supportsChannelUpstreamModelUpdate({
    type: formData.type,
    is_draft_multi_key: formData.multi_key_mode === 'multi_to_single',
  })
}
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  if (formDataSupportsUpstreamModelUpdate(formData)) {
    ...
    if (settingsObj.upstream_model_update_check_enabled !== true) {
      settingsObj.upstream_model_update_last_check_time = 0
    } else if (
      typeof settingsObj.upstream_model_update_last_check_time !== 'number'
    ) {
      settingsObj.upstream_model_update_last_check_time = 0
    }
  } else {
...
export function transformFormDataToUpdatePayload(
  ...
  channelId: number,
  options: {
    isPersistedMultiKey?: boolean
    isPersistedCodexMultiKey?: boolean
  } = {}
): Partial<Channel> {
  const isPersistedMultiKey =
    options.isPersistedMultiKey === true ||
    options.isPersistedCodexMultiKey === true
  const settingsFormData = isPersistedMultiKey
    ? ({
        ...formData,
        multi_key_mode: 'multi_to_single',
      } satisfies ChannelFormValues)
    : formData
  const payload: Partial<Channel> = {
    ...
    param_override: formData.param_override || null,
    header_override: formData.header_override || null,
    settings: buildSettingsJSON(settingsFormData),
    other: formData.other || '',
  }
...
export function formatModels(models: string[]): string {
  return [...new Set(models.map((model) => model.trim()).filter(Boolean))].join(
    ','
  )
}
...
export function formatGroups(groups: string[]): string {
  return [...new Set(groups.map((group) => group.trim()).filter(Boolean))].join(
    ','
  )
}
