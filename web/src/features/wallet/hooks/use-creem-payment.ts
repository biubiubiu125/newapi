...
export function useCreemPayment() {
  ...
  const processCreemPayment = useCallback(
    async (productId: string): Promise<PaymentInitiationResult> => {
      setProcessing(true)
      try {
        ...
        if (isApiSuccess(response) && response.data?.checkout_url) {
          if (!openCheckoutUrl(response.data.checkout_url)) {
          ...
          return {
            ok: true,
            tradeNo: response.data.trade_no || response.data.order_id,
          }
        }
