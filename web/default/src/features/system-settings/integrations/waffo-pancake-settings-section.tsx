...
export function WaffoPancakeSettingsSection({
  ...
  const verifyAndFetchCatalog = React.useCallback(
    async (
      ...
      let fetchedStores: CatalogStore[] | undefined
      try {
        ...
        if (
          ...
          fetchedStores = (body.data as { stores: CatalogStore[] }).stores ?? []
        } else {
      ...
      if (serial !== fetchSerialRef.current) return
      if (!fetchedStores) return

      setCatalog(fetchedStores)
      if (preselect) {
        onSelectedBindingChange({
        ...
        const boundStore = fetchedStores.find((s) =>
        ...
        if (boundStore && productID) {
          ...
          const storeWithProducts = fetchedStores.find(
