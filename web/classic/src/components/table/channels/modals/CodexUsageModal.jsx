...
const CodexUsageView = ({
  ...
  const displayedResetCredits =
    resetCredits ?? data?.rate_limit_reset_credits?.available_count;
  const canReset = Number(displayedResetCredits) > 0 && !resetting;
  const errorMessage =
    ...
    <div className='flex flex-col gap-4'>
      ...
      <div className='rounded-lg bg-semi-color-fill-0 p-4'>
        ...
        <div className='mt-2 flex flex-wrap items-center gap-2'>
          ...
          <Tag
            color={Number(displayedResetCredits) > 0 ? 'blue' : 'grey'}
            type='light'
            shape='circle'
          >
            {tt('重置次数：')}
            {resetCreditsLoading
              ? tt('加载中...')
              : Number.isFinite(Number(displayedResetCredits))
                ? String(displayedResetCredits)
                : '-'}
          </Tag>
...
const CodexUsageLoader = ({ t, record, initialPayload, onCopy }) => {
  ...
  const fetchResetCredits = useCallback(async () => {
    if (!recordId) {
      if (mountedRef.current) setResetCredits(null);
      return;
    }
  ...
  const handleResetUsage = useCallback(() => {
    ...
    Modal.confirm({
      title: tt('确认重置 Codex 用量'),
      content: tt('将消耗一次 Codex 重置次数，并刷新当前用量窗口。'),
      okText: tt('确定重置'),
      cancelText: tt('取消'),
      onOk: async () => {
        ...
        try {
          ...
          if (!res?.data?.success) {
            showError(res?.data?.message || tt('重置失败'));
            return;
          }
          showSuccess(tt('重置成功'));
          await fetchUsage();
  ...
  }, [fetchResetCredits, fetchUsage, recordId, resetting, tt]);
