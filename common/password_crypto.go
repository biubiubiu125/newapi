...
type PasswordReplayGuard func(keyID, ciphertextDigest string, expiresAt time.Time) error
...
var passwordReplayGuardState struct {
	sync.RWMutex
	claim PasswordReplayGuard
}

// SetPasswordReplayGuard installs the durable replay-claim implementation
// used by password decryption. A nil guard keeps the process-local fallback
// used before the database-backed application bootstrap is ready.
func SetPasswordReplayGuard(guard PasswordReplayGuard) {
	passwordReplayGuardState.Lock()
	passwordReplayGuardState.claim = guard
	passwordReplayGuardState.Unlock()
}
...
func LoadPasswordEncryptionPrivateKey(privateKeyPEM string) error {
	...
	passwordEncryptionState.used = make(map[string]time.Time)
	return nil
}
...
func DecryptPassword(ciphertextBase64 string, keyID string) (string, error) {
	...
	digest := sha256.Sum256(append([]byte(keyID+"\x00"), ciphertext...))
	digestKey := hex.EncodeToString(digest[:])
	now := time.Now()
	passwordReplayGuardState.RLock()
	claim := passwordReplayGuardState.claim
	passwordReplayGuardState.RUnlock()
	if claim != nil {
		if err := claim(keyID, digestKey, now.Add(5*time.Minute)); err != nil {
			return "", ErrPasswordEncryptionInvalid
		}
	...
	passwordEncryptionState.Lock()
	if passwordEncryptionState.used == nil {
		passwordEncryptionState.used = make(map[string]time.Time)
	}
	for usedDigest, expiresAt := range passwordEncryptionState.used {
		if !now.Before(expiresAt) {
			delete(passwordEncryptionState.used, usedDigest)
		}
	}
	if expiresAt, used := passwordEncryptionState.used[digestKey]; used && now.Before(expiresAt) {
