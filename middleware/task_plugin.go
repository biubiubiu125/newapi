...
// rebindTaskPluginEndpointForSelectedChannel switches the protocol parser and
// task adaptor to the candidate that owns the channel selected by Distribute.
// Shared protocol endpoints are initially pinned to a deterministic parser so
// model routing can happen before channel selection; the selected legacy
// channel is the authoritative provider identity for the actual submission.
func rebindTaskPluginEndpointForSelectedChannel(c *gin.Context, channel *model.Channel) {
	...
	var selected pluginruntime.ProtocolBinding
	for _, candidate := range pinned.Candidates {
		if candidate.Plugin == nil || !taskPluginCandidateOwnsChannel(candidate.Plugin.Meta, channel) {
			continue
		}
		selected = candidate
		break
	}
	if selected.Plugin == nil || selected.Plugin == pinned.Plugin {
		return
	}

	pinned.Plugin = selected.Plugin
	pinned.Protocol = selected.Protocol
	pinned.Operation = selected.Operation
	c.Set(pluginruntime.ContextKeyPinnedEndpoint, pinned)
	c.Set(pluginruntime.ContextKeyPinnedPlugin, pluginruntime.PinnedPlugin{
		Generation: pinned.Generation,
		Plugin:     selected.Plugin,
	})
	c.Set("expected_task_plugin_key", selected.Plugin.Meta.Key)
	c.Set("task_plugin_key", selected.Plugin.Meta.Key)
	c.Set("platform", selected.Plugin.Meta.Key)
}

func taskPluginCandidateOwnsChannel(meta pluginruntime.Meta, channel *model.Channel) bool {
	if channel == nil {
		return false
	}
	if channel.Type == constant.ChannelTypeTaskPlugin {
		return strings.TrimSpace(channel.GetSetting().TaskPluginKey) == meta.Key
	}
	return slices.Contains(meta.ChannelTypes, channel.Type)
}
