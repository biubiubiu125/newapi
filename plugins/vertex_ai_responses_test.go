...
func TestVertexAIExposesInlineVideoArtifact(t *testing.T) {
	source, err := builtinplugins.Source("vertex-ai")
	require.NoError(t, err)
	plugin, err := jsplugin.NewRegistry().RegisterFactory(source, jsplugin.Options{Key: "vertex-ai"})
	require.NoError(t, err)

	operation := map[string]any{
		"name": "projects/test/locations/us-central1/operations/op-1",
		"done": true,
	...
	require.Equal(t, []map[string]any{{"key": "video", "type": "video", "mimeType": "video/mp4"}}, artifacts)
	...
	require.True(t, ok)
	assert.Equal(t, "data:video/mp4;base64,aGVsbG8=", content["url"])
	assert.Equal(t, "GET", content["method"])

	t.Run("preserves case-insensitive media URL schemes", func(t *testing.T) {
		caseInsensitiveOperation := map[string]any{
			"name": "projects/test/locations/us-central1/operations/op-2",
			"done": true,
			"response": map[string]any{
				"video": "DATA:VIDEO/MP4;BASE64,aGVsbG8=",
			},
		}
		caseInsensitiveTask := map[string]any{
			"taskId": "task-public",
			"status": "SUCCESS",
			"data":   caseInsensitiveOperation,
		}

		value, err := plugin.Engine.Call(t.Context(), "listArtifacts", caseInsensitiveTask)
		require.NoError(t, err)
		encoded, err := common.Marshal(value)
		require.NoError(t, err)
		var artifacts []map[string]any
