...
function dataVideo(response) {
  ...
  if (!data || lowerData.startsWith("data:") || lowerData.startsWith("http://") || lowerData.startsWith("https://")) return data;
...
function artifactVideoURL(ctx) {
  return String(dataVideo((artifactData(ctx).response || {})) || "").trim();
}
function artifactVideoMimeType(ctx, url) {
  const response = artifactData(ctx).response || {};
  const videos = response.videos || [];
  const first = videos[0] || {};
  let enc = first.mimeType || first.encoding || response.encoding || "";
  if (!enc && String(url || "").toLowerCase().startsWith("data:")) {
    const header = String(url).split(",", 1)[0];
    enc = header.slice(5).split(";")[0];
  }
  if (!enc) return "";
  enc = String(enc).trim();
  return enc.includes("/") ? enc : "video/" + enc;
}
...
export function listArtifacts(task) {
  const url = artifactVideoURL(task);
  if (task.status !== "SUCCESS" || !url) return [];
  const artifact = { key: "video", type: "video" };
  const mimeType = artifactVideoMimeType(task, url);
...
export function buildContentRequest(ctx) {
  ...
  const request = { url: url, method: ctx.clientRequest.method };
  if (url.toLowerCase().startsWith("http://") || url.toLowerCase().startsWith("https://")) request.credentialless = true;
  return request;
}
