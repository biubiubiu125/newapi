...
func writeTaskArtifacts(c *gin.Context, task *model.Task, dashboard bool) {
	...
	artifacts, err := projectTaskArtifactsContext(c.Request.Context(), task)
	if err != nil {
...
func projectTaskArtifacts(task *model.Task) ([]relaychannel.TaskArtifact, error) {
	return projectTaskArtifactsContext(context.Background(), task)
}
...
func projectTaskArtifactsContext(ctx context.Context, task *model.Task) ([]relaychannel.TaskArtifact, error) {
	...
	adaptor, resolveErr := relay.GetTaskPluginAdaptorForTask(task)
	if resolveErr != nil || adaptor == nil {
		if len(storedArtifacts) > 0 {
			return storedArtifacts, nil
		}
		return nil, fmt.Errorf("%w: %v", errTaskArtifactPluginUnavailable, resolveErr)
	}
	...
	var err error
	if contextProvider, contextOK := adaptor.(relaychannel.TaskArtifactProviderContext); contextOK {
		artifacts, err = contextProvider.ListArtifactsContext(ctx, task)
	} else {
	...
	return mergeTaskArtifacts(artifacts, storedArtifacts), nil
}

func storedTaskArtifacts(task *model.Task) []relaychannel.TaskArtifact {
	stored := service.GetTaskArtifactStore().List(task)
	...
	for _, artifact := range stored {
		artifacts = append(artifacts, relaychannel.TaskArtifact{
			Key: artifact.Key, Type: artifact.Type, MimeType: artifact.MimeType,
		})
	}
	return artifacts
}

func mergeTaskArtifacts(primary, fallback []relaychannel.TaskArtifact) []relaychannel.TaskArtifact {
	if len(fallback) == 0 {
		return primary
	}
	...
	merged := append([]relaychannel.TaskArtifact(nil), primary...)
...
func initTaskArtifactAdaptor(task *model.Task) (relaychannel.TaskPluginAdaptor, error) {
	...
	adaptor.Init(&relaycommon.RelayInfo{
		ChannelMeta: &relaycommon.ChannelMeta{
			ChannelType: channelModel.Type, ChannelBaseUrl: baseURL, ApiKey: pluginKey,
...
func taskPluginPlatform(task *model.Task) constant.TaskPlatform {
	if task == nil || task.PrivateData.Execution == nil || task.PrivateData.Execution.TaskPlugin == nil {
		return ""
	}
	return constant.TaskPlatform(strings.TrimSpace(task.PrivateData.Execution.TaskPlugin.Key))
}
...
func resolveStoredTaskArtifact(c *gin.Context, task *model.Task, artifactKey string) bool {
	artifactStore := service.GetTaskArtifactStore()
	ref, resolveErr := artifactStore.Resolve(task, artifactKey)
	if resolveErr != nil {
		if errors.Is(resolveErr, service.ErrTaskArtifactNotFound) {
			writeTaskArtifactError(c, http.StatusNotFound, "artifact_not_found", "Task or artifact not found")
		} else {
			writeTaskArtifactError(c, http.StatusBadGateway, "artifact_storage_error", "Failed to resolve stored artifact")
		}
		return true
	}
	if ref == nil {
	...
	if serveErr := artifactStore.Serve(c, task, ref); serveErr != nil {
		if errors.Is(serveErr, service.ErrTaskArtifactNotFound) {
			writeTaskArtifactError(c, http.StatusNotFound, "artifact_not_found", "Task or artifact not found")
		} else {
			writeTaskArtifactError(c, http.StatusBadGateway, "artifact_storage_error", "Failed to serve stored artifact")
		}
	}
	return true
}
...
func TaskArtifactContent(c *gin.Context) {
	...
	if resolveStoredTaskArtifact(c, task, artifactKey) {
		return
	}
	if !taskHasPluginExecution(task) {
		...
		if legacyDescriptor, legacyOK, legacyErr := buildLegacyTaskMediaRequest(c, task); legacyErr != nil {
			writeTaskMediaProxyError(c, legacyErr)
			return
		} else if legacyOK {
			if err := proxyTaskMedia(c, task, legacyDescriptor); err != nil {
	...
	var selectedArtifact *relaychannel.TaskArtifact
	for _, artifact := range artifacts {
		if artifact.Key == artifactKey {
			artifactCopy := artifact
			selectedArtifact = &artifactCopy
			break
		}
	}
	...
	if contextProvider, contextOK := adaptor.(relaychannel.TaskContentRequestProviderContext); contextOK {
		descriptor, err = contextProvider.BuildContentRequestContext(c.Request.Context(), task, artifactKey, clientRequest)
	} else {
	...
	if err := proxyTaskMediaWithArtifact(c, task, descriptor, selectedArtifact); err != nil {
		writeTaskMediaProxyError(c, err)
	}
}
