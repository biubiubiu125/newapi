...
func TestRatioConfigExportsEffectiveExpressions(t *testing.T) {
	...
	withTieredBillingConfig(t, map[string]string{"sync-export": billing_setting.BillingModeTieredExpr}, map[string]string{"sync-export": `tier("base", p * 2)`})
...
func TestPricingSyncCompleteSourcesAndArrayFormats(t *testing.T) {
	...
	withTieredBillingConfig(t, map[string]string{"sync-already": billing_setting.BillingModeTieredExpr}, map[string]string{"sync-already": expression})
	...
	body, err := common.Marshal(map[string]any{"upstreams": []map[string]any{
	...
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/api/ratio_sync/fetch", bytes.NewReader(body))
