...
func TestEpayTopupNotifyAcksVerifiedNonSuccessWithoutFulfillment(t *testing.T) {
	gin.SetMode(gin.TestMode)
	setupPaymentCallbackGuardDB(t)
...
func TestSubscriptionEpayNotifyAcksVerifiedNonSuccessWithoutFulfillment(t *testing.T) {
	gin.SetMode(gin.TestMode)
	setupPaymentCallbackGuardDB(t)

	user := &model.User{Id: 921, Username: "sub_status_guard_user", Status: common.UserStatusEnabled}
	require.NoError(t, model.DB.Create(user).Error)
	plan := &model.SubscriptionPlan{
		Id:            814,
		Title:         "Status Guard Plan",
		PriceAmount:   9.99,
		Currency:      "CNY",
		DurationUnit:  model.SubscriptionDurationMonth,
		DurationValue: 1,
	...
	params := signedEpayCallback(map[string]string{
		...
		"type":         "alipay",
		"out_trade_no": order.TradeNo,
		"trade_no":     "gateway-sub-status-guard",
		"name":         "SUB:Status Guard Plan",
		"money":        "9.99",
		"trade_status": "TRADE_CLOSED",
	})
