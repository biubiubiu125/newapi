...
func channelErrorBodySecrets(channel *model.Channel) []string {
	...
	var parsed any
	if err := common.UnmarshalJsonStr(strings.TrimSpace(channel.Key), &parsed); err == nil {
...
func sanitizeUpstreamErrorBody(body []byte, secrets ...string) string {
	...
	var parsed any
	if err := common.UnmarshalJsonStr(bodyText, &parsed); err == nil {
		if redacted, err := common.Marshal(redactSensitiveErrorBody(parsed, secrets)); err == nil {
