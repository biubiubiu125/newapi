...
func WaffoWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo webhook 收到请求 path=%q client_ip=%s body_size=%d", c.Request.RequestURI, c.ClientIP(), len(bodyBytes)))

	// 验证请求签名
	if !wh.VerifySignature(bodyStr, signature) {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo webhook 验签失败 path=%q client_ip=%s body_size=%d", c.Request.RequestURI, c.ClientIP(), len(bodyBytes)))
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}
	...
	if err := common.Unmarshal(bodyBytes, &event); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo webhook 解析失败 path=%q client_ip=%s error=%q body_size=%d", c.Request.RequestURI, c.ClientIP(), err.Error(), len(bodyBytes)))
	...
	case core.EventPayment:
		...
		if err := common.Unmarshal(bodyBytes, &payload); err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo 支付回调载荷解析失败 event_type=%s client_ip=%s error=%q body_size=%d", event.EventType, c.ClientIP(), err.Error(), len(bodyBytes)))
