...
func RequestWaffoAmount(c *gin.Context) {
	...
	if req.Amount < waffoMinTopup {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": i18n.T(c, i18n.MsgTopupAmountMin, map[string]any{"Min": waffoMinTopup})})
		return
	}
	id := c.GetInt("id")
...
func RequestWaffoPay(c *gin.Context) {
	...
	if req.PayMethodIndex != nil {
		...
		if idx < 0 || idx >= len(methods) {
			...
			c.JSON(http.StatusOK, gin.H{"message": "error", "data": i18n.T(c, i18n.MsgPaymentMethodUnsupported)})
			return
		}
	...
	if notifyUrl == "" {
		...
		if err != nil {
			...
			topUp.Status = common.TopUpStatusFailed
			_ = topUp.Update()
			c.JSON(http.StatusOK, gin.H{"message": "error", "data": i18n.T(c, i18n.MsgTopupCallbackPublicRequired)})
			return
		}
		...
		topUp.Status = common.TopUpStatusFailed
		_ = topUp.Update()
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": i18n.T(c, i18n.MsgTopupCallbackNotLocal)})
		return
	}
