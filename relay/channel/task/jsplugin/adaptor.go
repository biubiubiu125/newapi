...
type taskResult struct {
	...
	UsageFacts       map[string]any `json:"usageFacts"`
	State            any            `json:"state"`
}
...
var errPluginAuthResolution = errors.New("plugin authentication resolution failed")
...
func (a *TaskAdaptor) ValidateRequestAndSetAction(c *gin.Context, info *relaycommon.RelayInfo) *dto.TaskError {
		...
		if pinned, ok := pinnedValue.(pluginruntime.PinnedEndpoint); ok && pinned.Plugin == a.plugin {
			if protocolValue, present := c.Get(pluginruntime.ContextKeyProtocolRequest); present {
				if protocolContext, valid := protocolValue.(pluginruntime.ProtocolRequestContext); valid {
					...
					if kind, kindOK := resolved["kind"].(string); !kindOK || kind != string(pluginruntime.RouteTypeSubmit) {
						return service.TaskErrorWrapperLocal(fmt.Errorf("final task plugin decoder must return a submit request"), "plugin_request_invalid", http.StatusBadRequest)
					}
					...
					setFinalTaskPluginRequestBody(c, resolved)
					if rawAction, present := resolved["action"]; present {
						action, valid := rawAction.(string)
						if !valid {
							return service.TaskErrorWrapperLocal(fmt.Errorf("final task plugin decoder returned an invalid action"), "plugin_request_invalid", http.StatusBadRequest)
						}
	...
	if _, err := a.buildSubmit(c, info); err != nil {
		if errors.Is(err, errPluginAuthResolution) {
...
func setFinalTaskPluginRequestBody(c *gin.Context, resolved map[string]any) {
	if c == nil {
		return
	}
	body, present := resolved["requestBody"]
	if present {
		c.Set("task_request", body)
	} else {
		delete(c.Keys, "task_request")
	}
	value, exists := c.Get(pluginruntime.ContextKeyRouteRequest)
	if !exists {
		return
	}
	request, ok := value.(pluginruntime.RouteRequestContext)
	if !ok {
		return
	}
	if present {
...
func (a *TaskAdaptor) EstimateBillingValidated(c *gin.Context, info *relaycommon.RelayInfo) (map[string]float64, error) {
	usageContext, err := a.submitContext(c, info)
	if err != nil {
		return nil, err
	}
...
func (a *TaskAdaptor) AdjustBillingOnSubmit(info *relaycommon.RelayInfo, taskData []byte) map[string]float64 {
	return a.AdjustBillingOnSubmitContext(context.Background(), info, taskData)
}

func (a *TaskAdaptor) AdjustBillingOnSubmitContext(ctx context.Context, info *relaycommon.RelayInfo, taskData []byte) map[string]float64 {
	ctx = pluginContextOrBackground(ctx)
	var data any
	if err := common.Unmarshal(taskData, &data); err != nil {
		data = string(taskData)
	}
	submitContext, err := a.submitContext(nil, info)
	if err != nil {
...
func (a *TaskAdaptor) AdjustBillingOnComplete(task *model.Task, result *relaycommon.TaskInfo) int {
	return a.AdjustBillingOnCompleteContext(context.Background(), task, result)
}

func (a *TaskAdaptor) AdjustBillingOnCompleteContext(ctx context.Context, task *model.Task, result *relaycommon.TaskInfo) int {
	ctx = pluginContextOrBackground(ctx)
	if !a.hasHook(ctx, "extractUsageOnComplete") {
		return 0
	}
	value, err := a.plugin.Engine.Call(ctx, "extractUsageOnComplete", jsonValue(task), jsonValue(result))
	if err != nil {
		return 0
	}
...
func (a *TaskAdaptor) ParseResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (*channel.TaskSubmitResponse, *dto.TaskError) {
	...
	submitContext, err := a.submitContext(c, info)
	if err != nil {
		return nil, service.TaskErrorWrapper(err, "plugin_auth_failed", http.StatusBadGateway)
	}
	value, err := a.plugin.Engine.Call(c.Request.Context(), "parseSubmitResponse", submitContext, map[string]any{"statusCode": resp.StatusCode, "headers": headers, "body": responseBody})
	if err != nil {
		logger.LogDebug(
			c,
	...
	if parsed.Immediate != nil {
		if !isKnownTaskStatus(parsed.Immediate.Status) {
			return nil, service.TaskErrorWrapperLocal(
				fmt.Errorf("plugin returned invalid immediate task status %q", parsed.Immediate.Status),
				"plugin_submit_response_invalid",
				http.StatusBadGateway,
			)
		}
		immediate = &relaycommon.TaskInfo{
			Code:             parsed.Immediate.Code,
			TaskID:           parsed.Immediate.TaskID,
			Status:           parsed.Immediate.Status,
			Progress:         parsed.Immediate.Progress,
			Reason:           parsed.Immediate.Reason,
			Url:              parsed.Immediate.URL,
			RemoteUrl:        parsed.Immediate.RemoteURL,
			CompletionTokens: positiveInt(parsed.Immediate.CompletionTokens),
		...
		if len(parsed.Immediate.UsageFacts) > 0 {
			...
			immediate.UsageFacts = values
			if units := positiveInt(values["upstreamUnits"]); units > 0 {
				if immediate.CompletionTokens == 0 {
					immediate.CompletionTokens = units
				}
				if immediate.TotalTokens == 0 {
					immediate.TotalTokens = units
				}
			} else {
				if immediate.CompletionTokens == 0 {
					if completionTokens, exists := values["completionTokens"]; exists {
						immediate.CompletionTokens = positiveInt(completionTokens)
					}
				}
				if immediate.TotalTokens == 0 {
					if totalTokens, exists := values["totalTokens"]; exists {
						immediate.TotalTokens = positiveInt(totalTokens)
					}
				}
			}
		}
		if parsed.Immediate.State != nil {
			if pluginState, present := encodePluginStateValue(parsed.Immediate.State); present {
				if len(pluginState) > maxTaskPluginPersistedJSONBytes {
					logger.LogWarn(c, fmt.Sprintf("task plugin %s rejected oversized immediate state (%d bytes)", a.plugin.Meta.Key, len(pluginState)))
				} else {
...
func (a *TaskAdaptor) FetchBatchTasks(baseURL, key string, tasks []*model.Task, proxy string) (*http.Response, error) {
	return a.FetchBatchTasksContext(context.Background(), baseURL, key, tasks, proxy)
}

func (a *TaskAdaptor) FetchBatchTasksContext(ctx context.Context, baseURL, key string, tasks []*model.Task, proxy string) (*http.Response, error) {
	ctx = pluginContextOrBackground(ctx)
	...
	queryCtx, err := a.batchQueryContext(key, baseURL, proxy, taskContexts)
	if err != nil {
		return nil, err
	}
	value, err := a.plugin.Engine.Call(ctx, "buildBatchQueryRequest", queryCtx, taskContexts)
	if err != nil {
		return nil, err
	}
	return a.doFetchDescriptor(ctx, baseURL, proxy, value)
}
...
func (a *TaskAdaptor) FetchTaskContext(ctx context.Context, baseURL, key string, task *model.Task, proxy string) (*http.Response, error) {
	ctx = pluginContextOrBackground(ctx)
	queryCtx, err := a.queryContext(task, key, baseURL, proxy)
	if err != nil {
		return nil, err
	}
	value, err := a.plugin.Engine.Call(ctx, "buildQueryRequest", queryCtx)
	if err != nil {
		return nil, err
	}
	return a.doFetchDescriptor(ctx, baseURL, proxy, value)
...
func (a *TaskAdaptor) ParseBatchResult(tasks []*model.Task, resp *http.Response, body []byte) (map[string]*service.BatchTaskResult, error) {
	return a.ParseBatchResultContext(context.Background(), tasks, resp, body)
}

func (a *TaskAdaptor) ParseBatchResultContext(ctx context.Context, tasks []*model.Task, resp *http.Response, body []byte) (map[string]*service.BatchTaskResult, error) {
	ctx = pluginContextOrBackground(ctx)
	started := time.Now()
	...
	value, err := a.plugin.Engine.Call(ctx, "parseBatchResult", queryCtx, input, hookHTTPResponse(resp))
	if err != nil {
		logger.LogDebug(ctx, "task_plugin subsystem=adaptor event=parse_batch_failed plugin=%q reason=hook_failed body_bytes=%d elapsed_ms=%d", a.plugin.Meta.Key, len(body), time.Since(started).Milliseconds())
		return nil, err
	}
	var parsed []struct {
		TaskID     string `json:"taskId"`
	...
	if err = convert(value, &parsed); err != nil {
		logger.LogDebug(ctx, "task_plugin subsystem=adaptor event=parse_batch_failed plugin=%q reason=invalid_result body_bytes=%d elapsed_ms=%d", a.plugin.Meta.Key, len(body), time.Since(started).Milliseconds())
		return nil, err
	}
	results := make(map[string]*service.BatchTaskResult, len(parsed))
	hasCompletionUsage := a.hasHook(ctx, "extractUsageOnComplete")
...
func (a *TaskAdaptor) ParseTaskResult(task *model.Task, resp *http.Response, body []byte) (*relaycommon.TaskInfo, error) {
	return a.ParseTaskResultContext(context.Background(), task, resp, body)
}

func (a *TaskAdaptor) ParseTaskResultContext(ctx context.Context, task *model.Task, resp *http.Response, body []byte) (*relaycommon.TaskInfo, error) {
	ctx = pluginContextOrBackground(ctx)
	started := time.Now()
	input := any(string(body))
	var decoded any
	...
	value, err := a.plugin.Engine.Call(ctx, "parseTaskResult", queryCtx, input, hookHTTPResponse(resp))
	if err != nil {
		logger.LogDebug(ctx, "task_plugin subsystem=adaptor event=parse_task_failed plugin=%q reason=hook_failed body_bytes=%d elapsed_ms=%d", a.plugin.Meta.Key, len(body), time.Since(started).Milliseconds())
		return nil, err
	}
	var parsed taskResult
	if err = convert(value, &parsed); err != nil {
		logger.LogDebug(ctx, "task_plugin subsystem=adaptor event=parse_task_failed plugin=%q reason=invalid_result body_bytes=%d elapsed_ms=%d", a.plugin.Meta.Key, len(body), time.Since(started).Milliseconds())
		return nil, err
	}
	...
	if pluginState, present := encodeReturnedPluginState(value); present {
		if len(pluginState) > maxTaskPluginPersistedJSONBytes {
			logger.LogWarn(ctx, fmt.Sprintf("task plugin %s rejected oversized poll state (%d bytes)", a.plugin.Meta.Key, len(pluginState)))
		} else {
...
func (a *TaskAdaptor) ConvertToOpenAIVideo(task *model.Task) ([]byte, error) {
	return a.ConvertToOpenAIVideoContext(context.Background(), task)
}

func (a *TaskAdaptor) ConvertToOpenAIVideoContext(ctx context.Context, task *model.Task) ([]byte, error) {
	ctx = pluginContextOrBackground(ctx)
	if task == nil {
...
func (a *TaskAdaptor) ListArtifacts(task *model.Task) ([]channel.TaskArtifact, error) {
	return a.ListArtifactsContext(context.Background(), task)
}

func (a *TaskAdaptor) ListArtifactsContext(ctx context.Context, task *model.Task) ([]channel.TaskArtifact, error) {
	ctx = pluginContextOrBackground(ctx)
	if !a.hasHook(ctx, "listArtifacts") {
		return nil, nil
	}
	artifactCtx, err := taskArtifactContext(task)
	if err != nil {
		return nil, err
	}
	value, err := a.plugin.Engine.Call(ctx, "listArtifacts", artifactCtx)
	if err != nil {
...
func (a *TaskAdaptor) BuildContentRequest(task *model.Task, artifactKey string, clientRequest channel.TaskArtifactClientRequest) (*channel.TaskContentRequest, error) {
	return a.BuildContentRequestContext(context.Background(), task, artifactKey, clientRequest)
}

func (a *TaskAdaptor) BuildContentRequestContext(ctx context.Context, task *model.Task, artifactKey string, clientRequest channel.TaskArtifactClientRequest) (*channel.TaskContentRequest, error) {
	ctx = pluginContextOrBackground(ctx)
	if !a.hasHook(ctx, "buildContentRequest") {
		return nil, nil
	}
	if a.info == nil {
	...
	artifactCtx["upstreamTaskId"] = task.GetUpstreamTaskID()
	artifactCtx["artifactKey"] = artifactKey
	...
	if rawURL == "" {
		return nil, fmt.Errorf("plugin returned an empty artifact request URL")
	}
	if strings.HasPrefix(strings.ToLower(rawURL), "data:") {
		if method != http.MethodGet && method != http.MethodHead {
			return nil, fmt.Errorf("data URL artifact requests must use GET or HEAD")
		}
		if len(descriptor.Headers) != 0 || descriptor.Body != nil || descriptor.Credentialless {
			return nil, fmt.Errorf("data URL artifact requests cannot contain headers, a body, or credentialless mode")
		}
		if err = validateInlineDataArtifactURL(rawURL); err != nil {
			return nil, err
		}
	} else if descriptor.Credentialless {
		if method != http.MethodGet && method != http.MethodHead {
...
func validateInlineDataArtifactURL(rawURL string) error {
	header, payload, ok := strings.Cut(rawURL, ",")
	if !ok || strings.TrimSpace(payload) == "" {
		return fmt.Errorf("plugin artifact data URL is invalid")
	}
	if !taskcommon.IsBase64DataURL(rawURL) {
		return fmt.Errorf("plugin artifact data URL must be base64")
	}
	mediaType := header[len("data:"):]
	if idx := strings.Index(mediaType, ";"); idx >= 0 {
		mediaType = mediaType[:idx]
	}
	if len(mediaType) > 255 || strings.ContainsAny(mediaType, "\r\n") {
		return fmt.Errorf("plugin artifact data URL media type is invalid")
	}
	for _, ch := range payload {
		if (ch >= 'A' && ch <= 'Z') || (ch >= 'a' && ch <= 'z') || (ch >= '0' && ch <= '9') || ch == '+' || ch == '/' || ch == '=' {
			continue
		}
		return fmt.Errorf("plugin artifact data URL payload is invalid")
	}
	return nil
}
...
func (a *TaskAdaptor) buildSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*requestDescriptor, error) {
	...
	value, err := a.plugin.Engine.Call(c.Request.Context(), "buildSubmitRequest", submitContext)
	if err != nil {
		logger.LogDebug(
			c,
...
func (a *TaskAdaptor) submitContext(c *gin.Context, info *relaycommon.RelayInfo) (map[string]any, error) {
	if info == nil {
		return nil, errors.New("relay info is unavailable")
	}
...
var _ channel.TaskPluginAdaptor = (*TaskAdaptor)(nil)
