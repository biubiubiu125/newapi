...
func TestOAuth2JWTAuthFailureRejectsSubmitBeforeHook(t *testing.T) {
	pluginAuthCache = sync.Map{}
	original := acquireAccessToken
	t.Cleanup(func() { acquireAccessToken = original; pluginAuthCache = sync.Map{} })
	acquireAccessToken = func(_ vertexcore.Credentials, _ string) (string, error) {
	...
	source := `
export const meta = {apiVersion:1,key:"oauth-fail-closed",name:"OAuth Fail Closed",version:"1.0.0",author:{name:"Test"},models:["m"],fetchMode:"per_task",auth:{type:"oauth2_jwt"}};
export function buildSubmitRequest(){throw new Error("hook must not run")}
export function parseSubmitResponse(){return {taskId:"1"}}
export function buildQueryRequest(){return {url:"https://example.com"}}
export function parseTaskResult(){return {status:"SUCCESS"}}
`
	...
	info := &relaycommon.RelayInfo{ChannelMeta: &relaycommon.ChannelMeta{
		...
		ApiKey:         `{"project_id":"project","client_email":"a@example.com","private_key":"secret"}`,
	}}
	adaptor.Init(info)
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/videos", nil)
	...
	assert.Equal(t, "plugin_auth_failed", taskErr.Code)
}
