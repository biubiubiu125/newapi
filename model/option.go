...
func updateOptionMap(key string, value string) (err error) {
	...
	if strings.HasSuffix(key, "Enabled") || key == "DefaultCollapseSidebar" || key == "DefaultUseAutoGroup" || key == "SMTPForceAuthLogin" || key == "SMTPInsecureSkipVerify" || key == "ReferralRequireApproval" {
	...
	case "BEpusdtPID":
		setting.BEpusdtPID = value
	case "BEpusdtSecretKey":
		setting.BEpusdtSecretKey = value
	case "BEpusdtCurrency":
	...
	case "BEpusdtAssetDisplayNames":
		setting.BEpusdtAssetDisplayNames = value
	...
	case "WaffoPancakeCurrency":
		...
		setting.WaffoPancakeCurrency = currency
	...
	case "ReferralRedemptionUSDToCNYRate":
		common.ReferralRedemptionUSDToCNYRate, _ = strconv.ParseFloat(value, 64)
	//case "ChatLink":
