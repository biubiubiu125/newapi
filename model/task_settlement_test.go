...
func TestCleanupTerminalTaskSettlementRecordsOnlyDeletesClosedRecords(t *testing.T) {
	...
	pendingLogTask := &Task{
		TaskID:           "task_cleanup_pending_log",
		Platform:         constant.TaskPlatformImage,
		Status:           TaskStatusSuccess,
		Progress:         "100%",
		SettlementStatus: TaskSettlementStatusSettled,
	}
	insertTask(t, pendingLogTask)
	...
	require.NoError(t, DB.Create([]*TaskSettlementRecord{
		...
		{TaskPrimaryID: pendingLogTask.ID, PublicTaskID: pendingLogTask.TaskID, Status: TaskSettlementRecordStatusApplied, CreatedAt: oldAt, UpdatedAt: oldAt, AppliedAt: oldAt, LogPayload: `{"request_id":"task_cleanup_pending_log"}`},
	...
	require.ElementsMatch(t, []string{"prepared_missing", "task_cleanup_pending", "task_cleanup_pending_log", "task_cleanup_recent"}, publicIDs)
}

func TestBeginTaskSettlementApplicationResumesAtomicApplyingRecord(t *testing.T) {
	prepareTaskSettlementRecordTest(t)
	task := &Task{
